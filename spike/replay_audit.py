"""Replay a recorded audit-agent trace by injecting recorded model and tool outputs: byte-identical replay
by injection. New file rather than a replay.py flag, for the same reason as record_audit.py: replay.py
stays untouched. Uses the trace's own recorded prompt (not a fixed module constant), since the audit
agent's prompt is parameterized by --target at record time.

    python spike/replay_audit.py [--run-id audit-001] [--storage local|aws]
"""
